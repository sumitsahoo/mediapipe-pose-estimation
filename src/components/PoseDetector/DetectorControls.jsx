...
const AnimatedButtonWrapper = ({ show, left, children }) => (
    <div
        className={`flex items-center justify-center overflow-hidden transition-all duration-300 ease-in-out ${show
                ? `w-12 h-12 opacity-100 scale-100 ${left ? "mr-2" : "ml-2"}`
...
const DetectorControls = ({
    ...
    <div
        className={`absolute bottom-8 flex justify-center items-center z-30 transition-all duration-300 rounded-full ${isDetecting
    ...
    >
        ...
        <div
            ...
            data-tip={isDetecting ? "Stop Detection" : "Start Detection"}
        >
            <button
                type="button"
                className={`btn btn-circle shadow-xl border-4 active:scale-95 transition-all duration-300 ${isDetecting
